...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if MODEL_NAME in ['RingGNN', '3WLGNN']:
        ...
        from train.train_edge_classification import train_epoch, evaluate_network
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                epoch_train_loss, epoch_train_acc, optimizer = train_epoch(model, optimizer, device, graph, train_edges, params['batch_size'], epoch)
                
                epoch_val_loss, epoch_val_acc  = evaluate_network(
                    model, device, graph, train_edges, val_edges, val_edges_neg, test_edges, test_edges_neg, params['batch_size'], epoch)
                
                _, epoch_test_acc = evaluate_network(
                    model, device, graph, train_edges, val_edges, val_edges_neg, test_edges, test_edges_neg, params['batch_size'], epoch, test=True)

                epoch_train_losses.append(epoch_train_loss)
                epoch_train_accs.append(epoch_train_acc)
                epoch_val_losses.append(epoch_val_loss)
                epoch_val_accs.append(epoch_train_acc)
